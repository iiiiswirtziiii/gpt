...
# If KIT_CLAIMS_FILE env var is set, we use it.
# Otherwise, load kit_claims.txt from the same folder as this file (portable for Railway/local).
_DEFAULT_CLAIMS_PATH = os.path.join(os.path.dirname(__file__), "kit_claims.txt")
KIT_CLAIMS_FILE = os.getenv("KIT_CLAIMS_FILE", _DEFAULT_CLAIMS_PATH)
...
def load_kit_claims_text() -> None:
    ...
    if not os.path.exists(KIT_CLAIMS_FILE):
        print(f"[KIT CLAIMS] File not found: {KIT_CLAIMS_FILE} (cwd={os.getcwd()})")
        return
