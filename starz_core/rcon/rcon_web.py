...
# -------------------------
# Throttled debug: playerlist extraction
# -------------------------
_PLAYERLIST_DEBUG_EVERY_SECONDS = 60.0
_last_playerlist_debug_ts: Dict[str, float] = {}


def _maybe_debug_playerlist(server_key: str, players: list) -> None:
    """
    Prints playerlist extraction info at most once per N seconds per server.
    """
    now = time.time()
    last = _last_playerlist_debug_ts.get(server_key, 0.0)
    if now - last < _PLAYERLIST_DEBUG_EVERY_SECONDS:
        return
    _last_playerlist_debug_ts[server_key] = now

    # Normalize names for preview
    names: list[str] = []
    for p in players or []:
        if isinstance(p, dict):
            n = p.get("DisplayName") or p.get("displayName") or p.get("name")
            if n:
    ...
    preview = ", ".join(names[:5]) if names else ""
    print(f"[PLAYERLIST:{server_key}] extracted={len(names)}" + (f" sample=[{preview}]" if preview else ""))


def _extract_players_from_playerlist_response(resp: dict) -> list:
    """
    Return players as list[str] or list[dict] compatible with update_connected_players().

    Tries multiple formats:
      1) resp["Players"] / resp["players"] list
      2) resp["Message"] contains JSON with Players
      3) resp["Message"] contains plain text (best-effort parse)
    """
    ...
    if msg.startswith("{") or msg.startswith("["):
        try:
            ...
            if isinstance(j, dict):
                pl = j.get("Players") or j.get("players")
    ...
    # 3) Plain text formats (best effort)
    out: list[str] = []

    # Common "Players ...: Name1, Name2"
    lower = msg.lower()
    if "players" in lower and ":" in msg:
        try:
            tail = msg.split(":", 1)[1]
    ...
    # Multi-line: take non-empty lines, skip obvious headers, try to grab a name token
    for line in msg.splitlines():
        ...
        l = line.lower()
        if "players" in l or "steam" in l or l == "id" or l.startswith("id "):
            continue
        ...
        # If first token is numeric, name likely starts at token 2
        if chunks[0].isdigit() and len(chunks) >= 2:
            out.append(chunks[1])
        else:
            out.append(chunks[0])
        ...
        async with self._lock:
            ...
            try:
                ...
                raise asyncio.TimeoutError(
                    f"Timeout waiting for RCON response ({self.name}) for: {command}"
                )
...
async def run_rcon_command(command: str, client_key: str = "s1") -> Optional[dict]:
    ...
    try:
        ...
        # ✅ Feed playerlist into PrintPos/TP tracker (supports multiple response formats)
        if command.strip().lower().startswith("playerlist"):
            try:
                from starz_printpos.tp_tracker import update_connected_players
                players = _extract_players_from_playerlist_response(resp)
                update_connected_players(client_key, players)
                _maybe_debug_playerlist(client_key, players)
            except Exception as e:
                print(f"[PRINTPOS] update_connected_players failed for {client_key}: {e}")
